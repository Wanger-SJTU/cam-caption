...
from utils.pycocotools.coco import COCO as COCOapi
...
class ClsDataSet(object):
    ...
    def next_batch(self):
        ...
        if self.is_train:
            cls_ids = self.cls_ids[current_idxs]
            self.current_idx += self.batch_size
            return image_files, cls_ids
...
def prepare_cls_data(config):
    """ Prepare the data for training the model. """
    coco = COCOapi(config.train_cls_file)
    ...
    cls_ids = [getCls_ids(coco, image_id) for image_id in image_ids]
    dataset = ClsDataSet(image_ids, image_files, cls_ids, config.batch_size)
    return dataset


def getCls_ids(coco, img_id):
    cls_id = [0]*90
    annIds = coco.getAnnIds(imgIds=img_id, iscrowd=None)
    anns = coco.loadAnns(annIds)
    for item in anns:
        cls_id[item['category_id']-1] = 1
    return cls_id
